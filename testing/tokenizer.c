...
char **tokenizer(char *userInput)
{
	...
	if(!userInput)			/*that case no input from user*/
	{
		return (NULL);
	 				...
	 				/*userInput = "ls -la"*/
	tmp = strdup(userInput); 	/*tmp = "ls -la"*/
	...
	arrayOfTokens = malloc(sizeof(char *) * (count + 1)); /*malloc 3 string due to NULL*/
	...
	while(token)
	{
		arrayOfTokens[i++] = strdup(token);	/*arrayOfTokens = {"ls","-la",NULL}*/
